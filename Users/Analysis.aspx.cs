...
public partial class Users_Details : System.Web.UI.Page
{
    ...
    void getAmazonData()
    {
        ...
        con.Open();
        cmd = new SqlCommand();
        cmd.CommandText = "update AnalysisReviewAll set count_positive=" + tpc + " , count_negative=" + tnc + " where ctype='Amazon'";
        cmd.Connection = con;
        cmd.ExecuteNonQuery();
        con.Close();
    }
    ...
    void getRelienceDigitalData()
    {
        ...
        con.Open();
        cmd = new SqlCommand();
        cmd.CommandText = "select review from Relience_Digital_Reviews where pid='" + Request.QueryString["pid"] + "'";
        cmd.Connection = con;
        SqlDataReader dr = cmd.ExecuteReader();
        int tnc = 0, tpc = 0, tnec = 0;
        ...
        con.Open();
        cmd = new SqlCommand();
        cmd.CommandText = "update AnalysisReview_Relience_Digital    set count=" + tpc + " where ctype='Positive'";
        ...
        con.Open();
        cmd = new SqlCommand();
        cmd.CommandText = "update AnalysisReview_Relience_Digital set count=" + tnc + " where ctype='Negative'";
        cmd.Connection = con;
        cmd.ExecuteNonQuery();
        con.Close();
        ...
        con.Open();
        cmd = new SqlCommand();
        cmd.CommandText = "update AnalysisReviewAll set count_positive=" + tpc + " , count_negative=" + tnc + " where ctype='Relience_Digital'";
        cmd.Connection = con;
        cmd.ExecuteNonQuery();
        con.Close();
    }
    ...
    void getFlipkartData()
    {
        ...
        con.Open();
        cmd = new SqlCommand();
        cmd.CommandText = "update AnalysisReviewAll set count_positive=" + tpc + " , count_negative=" + tnc + " where ctype='Flipkart'";
        cmd.Connection = con;
        cmd.ExecuteNonQuery();
        con.Close();
    }
    ...
    private void SuggestBestPlatform()
    {
        string connStr = ConfigurationManager.ConnectionStrings["FunctionalConnectionString"].ConnectionString;
        using (SqlConnection conn = new SqlConnection(connStr))
        {
            string query = @"
    SELECT ctype 
    FROM AnalysisReviewAll 
    WHERE count_positive = (
        SELECT MAX(count_positive) FROM AnalysisReviewAll
    )";
            ...
            while (reader.Read())
            {
                platforms.Add(reader["ctype"].ToString());
            }

            if (platforms.Count > 0)
            {
                string platformList = string.Join(", ", platforms);
                lblRecommendation.Text = "Based on reviews, you should consider buying from <strong>"+ platformList + "</strong>.";
            }
            else
            {
                lblRecommendation.Text = "No review data available for recommendation.";
            }
        }
    }
